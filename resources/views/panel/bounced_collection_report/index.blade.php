...
<div class="container-xxl flex-grow-1 container-p-y">
          ...
          <select class="form-select" name="myprod">
            ...
            @if(isset($payment_methods) && $payment_methods->count()>0)
            	@foreach($payment_methods as $key => $payment_method)
                	<option value="{{$payment_method->id}}">{{$payment_method->payment_methods}}</option>
                @endforeach
            @endif
          </select>
        </div>
        <div class="col-md-2">
          <select class="form-select" name="mystat">
            <option value="">Select Payment Status</option>
            <option selected="selected" value="2">Bounced</option>
  ...
  <div class="card">
    <h5 class="card-header">Collection Report Bounced</h5>
...
<script type="text/javascript">	
	...
	function filterData(type = null){
		...
		$.ajax({
			...
			url: "{{ url('/panel/bounced_collection_report_paginate') }}",
