@if($records->count()>0)
  ...
  <td>
    ...
    <a style="cursor:pointer" onclick="changeStatus('holidays','{!!$row->id!!}');" id="status_{{$row->id}}" class="badge {{$class}} me-1">{{$label}}</a>
  ...
  <td>
      ...
      <div class="dropdown-menu"> 
      	<a class="dropdown-item" style="cursor:pointer" href="{{url('panel/edit-holiday',base64_encode($row->id))}}"><i class="icon-base ti tabler-pencil me-1"></i> Edit</a> 
        @if(Session::get('admin_type') == 'Admin')

    <a class="dropdown-item" onclick="deleteData('holidays','{{ $row->id }}');" href="javascript:void(0);"><i class="icon-base ti tabler-trash me-1"></i> Delete</a>
