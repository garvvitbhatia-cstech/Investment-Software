...
class BankCollectionReportController extends Controller {
    ...
    public function getList(Request $request){
		...
		return view('/panel/bank_collection_report/index',compact('PREV','products','payment_methods'));
    }
    ...
    public function listPaginate(Request $request){
		...
		$query->where('mel_investment.payment_method', '!=', 1);		
		$query->orderBy('mel_investment.id','desc');
		$records = $query->paginate(20);
		$sum_investment_amount = $query->sum('investment_amount');
		$sum_reg_fee = $query->sum('reg_fee');
        return view('/panel/bank_collection_report/paginate', compact('records','PREV','sum_investment_amount','sum_reg_fee'));
    }
