...
class TodaysMaturityController extends Controller {
    ...
    public function getList(Request $request){
		...
		return view('/panel/todays_maturity_report/index',compact('PREV','products','payment_methods'));
    }
    ...
    public function listPaginate(Request $request){
		...
		if(!empty($request->input('from_date')) || !empty($request->input('to_date'))){
			...
			$query->whereRaw("DATEDIFF('".NOW()."',mel_investment.maturity_date) = 0");
		}
		
		if($PREV == 2){
			//$findallcustomer=dbQuery($dbConn,"Select a.id as myid,a.*,b.cust_name,b.udaid,b.id as customr_id,c.payment_methods,d.prod_name,d.id,datediff(a.maturity_date,NOW()) as mydiff,e.* from mel_investment a inner join master_customer b on a.cust_id=b.id inner join master_payment_method c on a.payment_method=c.id inner join master_product d on a.product_id=d.id inner join master_branch e on a.branch_id=e.id where 1   $mystr $mystr2 and a.status='1' and a.is_paid='0' and a.branch_id='" . $_SESSION['BRID'] . "' order by a.id desc");
			$query->where('mel_investment.branch_id', $BRID);
		}
		if($PREV == 1 || $PREV == 3){
			//$findallcustomer=dbQuery($dbConn,"Select a.id as myid,a.*,b.cust_name,b.udaid,b.id as customr_id,c.payment_methods,d.prod_name,d.id,datediff(a.maturity_date,NOW()) as mydiff,e.* from mel_investment a inner join master_customer b on a.cust_id=b.id inner join master_payment_method c on a.payment_method=c.id inner join master_product d on a.product_id=d.id inner join master_branch e on a.branch_id=e.id where 1   $mystr $mystr2 and a.status='1' and a.is_paid='0'  order by a.id desc");
		}
        ...
        return view('/panel/todays_maturity_report/paginate', compact('records','PREV','sum_investment_amount','sum_reg_fee'));
    }
